...
int main() {
    ...
    while (!quit){
        ...
        float G = 50;
        for (int i = 0; i < 1; i++) {
            tree = list.update3(window, trackedEntity, (1.0)/frameRate, 10, G, tree);//, 10); // Updates entity positions, checks for out of bounds entities, and checks for & calculates collisions
        }
