...
QuadTree* allEntities::update3(SDL_Window *window, entity* trackedRect, double dTime, int windowsize, float G, QuadTree* tree) {
    ...
    *c = 0;
    ...
    for (int i = 0; i < size; i++) {
        rects[i]->reset();
        barnesHut(window, dTime, G, tree, tree->root, rects[i], c);
    }
   ...
   // std::cout << "Reachedhere!\n";
    ...
    for (int i = 0; i < size; i++) {
        ...
        //std::cout << "Reachedhere!" << rects[i]->mass << "\n";
    }
...
void allEntities::barnesHut(SDL_Window *window, double dTime, float G, QuadTree* tree, node* b, entity* a, int *c) {
    ...
    if (!b) {
        ...
        barnesHut(window, dTime, G, tree, b->pointers[0], a, c);
        barnesHut(window, dTime, G, tree, b->pointers[1], a, c);
        barnesHut(window, dTime, G, tree, b->pointers[2], a, c);
        barnesHut(window, dTime, G, tree, b->pointers[3], a, c);
    }
